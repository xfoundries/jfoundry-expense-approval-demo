...
@Transactional
class ExpenseClaimViewReaderTest extends PostgreSqlIntegrationTest {
    ...
    @Autowired ExpenseClaimViewReader viewReader;
    ...
    void ownedClaimsCanBeFilteredAndPaged() {
        ...
        PageResult<ClaimSummary> page = viewReader.findOwned(
    ...
    void approvalQueuesContainOnlyTheirCurrentStage() {
        ...
        assertThat(viewReader.findPendingManager(new PageQuery(0, 20)).items())
                .extracting(ClaimSummary::id).containsExactly("claim-manager");
        assertThat(viewReader.findPendingFinance(new PageQuery(0, 20)).items())
    ...
    void detailContainsItemsAndOrderedActionHistory() {
        ...
        ClaimDetail detail = viewReader.findDetail(claim.id()).orElseThrow();
    ...
    void queryDerivesPaymentStateAndIncludesProjectedResult() {
        ...
        Map<String, ClaimSummary> summaries = viewReader.findOwned(
        ...
        ClaimDetail detail = viewReader.findDetail(ExpenseClaimId.of("claim-failed")).orElseThrow();
