...
class ExpenseClaimQueryServiceTest {
    ...
    @Mock ExpenseClaimViewReader viewReader;
    private ExpenseClaimQueryService service;

    @BeforeEach
    void setUp() {
        service = new ExpenseClaimQueryService(viewReader);
    }
    ...
    void queueRequiresMatchingRoleBeforeQueryingPort() {
        ...
        verify(viewReader, never()).findPendingManager(new PageQuery(0, 20));
    }
    ...
    void missingDetailBecomesNotFound() {
        ...
        ApprovalActor employee = new ApprovalActor(UserId.of("employee-1"), ApprovalRole.EMPLOYEE);
        when(viewReader.findDetail(id)).thenReturn(Optional.empty());
    ...
    void employeeCannotViewAnotherClaimantsDetail() {
        ...
        ApprovalActor employee = new ApprovalActor(UserId.of("employee-1"), ApprovalRole.EMPLOYEE);
        when(viewReader.findDetail(id)).thenReturn(Optional.of(detail("employee-2")));
